    ...
    useEffect(() => {
        ...
        const updateDots = (progress: number) => {
            dots.forEach((dot, i) => {
                ...
                // Calculate color based on x and y position for gradient effect
                const colorX = Math.floor((x + radius) / (2 * radius) * 255);
                const colorY = Math.floor((y + radius) / (2 * radius) * 255);
                const color = `rgb(${colorX}, ${colorY}, 255)`;
